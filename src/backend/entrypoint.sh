...
alembic upgrade head # apply pending schema changes

printf '%s\n' "[entrypoint] Seeding synthetic data..."
python data/seed_db.py # data seed is idempotent

printf '%s\n' "[entrypoint] Starting application server..."
exec env uvicorn app.main:app --host 0.0.0.0 --port 8000
